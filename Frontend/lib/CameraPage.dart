...
import 'package:smart_gains/StatisticsTab.dart';
...
class _CameraPageState extends State<CameraPage> {
  ...
  _processMessage(message) {
    ...
    if (data["type"] == "REP_DONE") {
      // if (_reps != data["count"].toString()) {
      //   _processRequest(data["count"].toString());
      //   //_processRequest(data["feedback_list"].join(". "));
      // }
      _processRequest(_reps.toString());
      WidgetsBinding.instance.addPostFrameCallback((_) {
        setState(() {
          //var rep = Rep(feedback: data["feedback_list"], id: data["count"], gif: "");
          //if (!reps.contains(rep)) {
          _reps = data["count"];
          _feedback = data["feedback_list"].join(". ");
          //reps.add(rep);
          //}
        });
      });
